...
class Station:
    ...
    def update_position(self, new_x, new_y):
        self.x = new_x
        self.y = new_y
        self.area = (self.x - self.coverage_radius, self.y - self.coverage_radius,
                     self.x + self.coverage_radius, self.y + self.coverage_radius)


def generate_stations(num_stations, coverage_radius, area_size):
    stations = []
    for i in range(num_stations):
        name = f"İstasyon {chr(65 + i)}"  # Generate names like Station A, Station B, etc.
...
def optimize_algorithm():
    ...
    stations = generate_stations(num_stations, coverage_radius, area_size)
    total_time = 5
    time_step = 1
    ...
    if algorithm_choice == '1':
        ...
        for t in range(0, 6, 1):  # 0'dan 5'e kadar 1'er saniye aralıklarla
        ...
        # İstasyon pozisyon geçmişini alalım
        station_position_history = []

        for t in range(0, 6):  # 0'dan 5'e kadar saniyeler arası
            station_position_history.append([(station.x, station.y) for station in stations])  # Mevcut pozisyonları geçmişe ekle
            # İstasyon pozisyonlarını güncelle (örnek olarak her seferinde rastgele güncelleme yapalım)
            for station in stations:
                new_x = random.randint(max(0, station.x - 10), min(area_size, station.x + 10))  # 10 birimlik rastgele güncelleme
                new_y = random.randint(max(0, station.y - 10), min(area_size, station.y + 10))  # 10 birimlik rastgele güncelleme
                station.update_position(new_x, new_y)

        result_text.insert(tk.END, "\nİstasyon Pozisyon Geçmişi:\n")
        for t, snapshot in enumerate(station_position_history):
            result_text.insert(tk.END, f"\nZaman {t}:\n")
            for i, (x, y) in enumerate(snapshot):
                station_name = stations[i].name
                result_text.insert(tk.END, f"Istasyon {station_name}: Konum: ({x:.2f}, {y:.2f})\n")

        result_text.config(state=tk.DISABLED)

        results = (
        f"\nSENARYO 3 PSO ALGORTİMASI SONUÇLARI\n"
        f"\nPSO ile Elde Edilen En İyi Uygunluk Değeri: {best_fitness}\n"
        f"\nOrtalama Mesafe: {average_distance}\n"
        f"\nOrtalama Batarya Kullanımı: {average_battery_usage}\n"
        f"\nGeçen Süre: {elapsed_time:.4f} saniye\n"
        ...
        results = (
        f"\nSENARYO 3 GWO ALGORTİMASI SONUÇLARI\n"
        f"\nGWO ile Elde Edilen En İyi Uygunluk Değeri: {best_fitness_gwo}\n"
        f"\nOrtalama Mesafe: {average_distance}\n"
        f"\nOrtalama Batarya Kullanımı: {average_battery_usage}\n"
        f"\nGeçen Süre: {elapsed_time:.4f} saniye\n"
    ...
    if algorithm_choice == '3':
        ...
        results = (
        f"\nSENARYO 3 ACO ALGORTİMASI SONUÇLARI\n"
        f"\nACO ile Elde Edilen En İyi Uygunluk Değeri: {best_fitness_aco}\n"
        f"\nOrtalama Mesafe: {average_distance}\n"
        f"\nOrtalama Batarya Kullanımı: {average_battery_usage}\n"
        f"\nGeçen Süre: {elapsed_time:.4f} saniye\n"
        ...
        results = (
        f"\nSENARYO 3 GA ALGORTİMASI SONUÇLARI\n"
        ...
        results = (
        f"\nSENARYO 3 ABC ALGORTİMASI SONUÇLARI\n"
        ...
        results = (
        f"\nSENARYO 3 GOA ALGORTİMASI SONUÇLARI\n"
     ...
     # DEA için gerekli parametreler
        ...
        results = (
        f"\nSENARYO 3 DEA ALGORTİMASI SONUÇLARI\n"
        f"\nDEA ile Elde Edilen En İyi Uygunluk Değeri: {best_fitness}\n"
        f"\nOrtalama Mesafe: {average_distance}\n"
        f"\nOrtalama Batarya Kullanımı: {average_battery_usage}\n"
        f"\nGeçen Süre: {elapsed_time:.4f} saniye\n"
...
tk.Label(algorithm_frame, text="HAREKETLİ DRONE, HAREKETLİ YER İSTASYONU", bg='#282c34', fg='#ff6347', font=('calibri', 16, 'bold')).grid(row=0, column=0, columnspan=2, pady=5)
