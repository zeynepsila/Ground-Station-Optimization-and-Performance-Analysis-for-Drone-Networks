...
def generate_drones(drone_positions):
    ...
    for i, position in enumerate(drone_positions):
        x, y = position
        model_data = drone_models[i]
...
# İstasyon konumları ve isimleri
station_positions = [
    ("İstasyon A", (200, 200)),
    ("İstasyon B", (500, 200)),
    ("İstasyon C", (200, 500)),
...
# Önceden tanımlanmış konum listesi
drone_positions =  [
    ((100, 100)),
    ((250, 250)),
    ((350, 500)),
...
# Her bir dron için bir konum seçin
i = 0  # Dronun indeksi
...
# İstasyonları ve drone'ları oluştur
stations = generate_stations(station_positions, coverage_radius)
drones = generate_drones(drone_positions)
...
def optimize_algorithm():
    ...
    max_iterations = 10000  # Sabit iterasyon sayısı her algoritma için
    fitness_values = []  # fitness_values değişkenini başlangıçta tanımlayın
    # optimize_algorithm fonksiyonunun içinde unassigned_drones listesini tanımlayın
    unassigned_drones = []
    ...
    if algorithm_choice == '1':
        ...
        results = (
        f"\nSENARYO 1 PSO ALGORTİMASI SONUÇLARI\n"
        f"\nPSO ile Elde Edilen En İyi Uygunluk Değeri: {best_fitness}\n"
        f"\nOrtalama Mesafe: {average_distance}\n"
        f"\nOrtalama Batarya Kullanımı: {average_battery_usage}\n"
        f"\nGeçen Süre: {elapsed_time:.4f} saniye\n"
        ...
        results = (
        f"\nSENARYO 1 GWO ALGORTİMASI SONUÇLARI\n"
        f"\nGWO ile Elde Edilen En İyi Uygunluk Değeri: {best_fitness_gwo}\n"
        f"\nOrtalama Mesafe: {average_distance}\n"
        f"\nOrtalama Batarya Kullanımı: {average_battery_usage}\n"
        f"\nGeçen Süre: {elapsed_time:.4f} saniye\n"
    ...
    if algorithm_choice == '3':
        ...
        result_text.config(state=tk.DISABLED)
        results = (
        f"\nSENARYO 1 ACO ALGORTİMASI SONUÇLARI\n"
        f"\nACO ile Elde Edilen En İyi Uygunluk Değeri: {best_fitness_aco}\n"
        f"\nOrtalama Mesafe: {average_distance}\n"
        f"\nOrtalama Batarya Kullanımı: {average_battery_usage}\n"
        f"\nGeçen Süre: {elapsed_time:.4f} saniye\n"
        ...
        results = (
        f"\nSENARYO 1 GA ALGORTİMASI SONUÇLARI\n"
        ...
        results = (
        f"\nSENARYO 1 ABC ALGORTİMASI SONUÇLARI\n"
        ...
        results = (
        f"\nSENARYO 1 GOA ALGORTİMASI SONUÇLARI\n"
        ...
        filename = "GOA_results1.txt"
        save_results_to_file(filename, results)      

    # optimize_algorithm fonksiyonu içinde kullanımı
     ...
     # DEA için gerekli parametreler
        ...
        for _ in range(100):
            ...
            results = (
            f"\nSENARYO 1 DEA ALGORTİMASI SONUÇLARI\n"
            f"\nDEA  ile Elde Edilen En İyi Uygunluk Değeri: {best_fitness}\n"
            f"\nOrtalama Mesafe: {average_distance}\n"
            f"\nOrtalama Batarya Kullanımı: {average_battery_usage}\n"
            f"\nGeçen Süre: {elapsed_time:.4f} saniye\n"
            ...
            filename = f"DEA_results{_ + 1}.txt"
            save_results_to_file(filename, results)
...
tk.Label(algorithm_frame, text="SABİT DRONE, SABİT YER İSTASYONU", bg='#282c34', fg='#ff6347', font=('calibri', 16, 'bold')).grid(row=0, column=0, columnspan=2, pady=5)
...
result_frame = tk.Frame(root, bg='#282c34', bd=2, width=2000, height=2000)
